...
function createLineReader(readStream, options, creationCb) {
  ...
  function nextLine(cb) {
    ...
    function getLine(err) {
      ...
      var length = separatorIndex + separatorLen;

      bufferStr = bufferStr.substring(length);
      separatorIndex = -1;
      cb(undefined, offset, ret);
      offset += length;
    }
...
function open(filenameOrStream, options, cb) {
  ...
  if (typeof filenameOrStream.read == 'function') {
    ...
    readStream = fs.createReadStream(filenameOrStream, {start: start});
  } else {
...
function eachLine(filename, options, iteratee, cb) {
  ...
  open(filename, options, function(err, reader) {
    ...
    function readNext() {
      reader.nextLine(function(err, offset, line) {
        if (err) {
          finish(err);
        }
        ...
        if (asyncIteratee) {
          iteratee(line, offset, last, continueCb);
        } else {
          if (iteratee(line, offset, last) !== false) {
            newRead();
          } else {
            finish();
          }
